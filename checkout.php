<?php
    ...
    if(isset($_POST['place_order'])){
        ...
        $varify_cart = $conn->prepare("SELECT * FROM products WHERE id=? LIMIT 1");
        $varify_cart->execute['$user_id'];

        if(isset($_GET['get_id'])){
            $get_product = $conn->prepare("SELECT * FORM products WHERE id=? LIMIT 1");
            ...
            if($get_product->rowCount() > 0){
                ...
                $warning_msg[] = 'sthg went wrong';
            }
        }elseif($varify_cart->rowCount()>0){
            while($f_cart = $varify_cart->fetch(PDO::FETCH_ASSOC)){
                $insert_order = $conn->prepare("INSERT INTO orders(id ,user_id , name, number, email, address , address_type, $method, product_id, price, qty) VALUES(?,?,?,?,?,?,?,?,?,?,?)");
                $insert_order->execute([unique_id(), $user_id, $name, $number, $email, $address, $address_type, $method , $fetch_p['id'], $fetch_cart['price'],$f_cart['qty']]);
                header('location: order.php');
            }
            if($insert_order){
